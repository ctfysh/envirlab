...
function doOptimizer(){
	...
	var grid = Ext.create('Ext.grid.Panel', {
	    ...
	    columns: [
	        ...
	        {header: getText('最小界限'), sortable: false, menuDisabled: true, dataIndex: 'minBound',width:130,
	            editor: {
	                xtype: 'numberfield',
	                allowBlank: false,
					allowDecimals: true,
	        ...
	        {header: getText('最大界限'), sortable: false, menuDisabled: true, dataIndex: 'maxBound',width:130,
	            editor: {
	                xtype: 'numberfield',
	                allowBlank: false,
					allowDecimals: true,
	        ...
	        {header: getText('准确性'), sortable: false, menuDisabled: true, dataIndex: 'desiredAccuracy',width:130,
	            editor: {
	                xtype: 'numberfield',
	                allowBlank: false,
					step: 0.1,
	...
	var generalForm = new Ext.FormPanel({
		...
		margin: '11 11 11 11',
			...
			Ext.create('Ext.form.field.ComboBox', {
				fieldLabel: getText('目标'),
				name: 'optGoal',
				id: 'optGoal',
				queryMode: 'local',
				store: [["Minimize", getText("最小化")], ["Maximize", getText("最大化")]],
				forceSelection:true,
				allowBlank:false,
				value: mySetting.getAttribute("OptimizerGoal")?mySetting.getAttribute("OptimizerGoal"):"Minimize"
			}),
			Ext.create('Ext.form.field.ComboBox', {
				fieldLabel: getText('目标类型'),
				name: 'optGoalType',
				id: 'optGoalType',
				queryMode: 'local',
				store: [["Integral of Value", getText("整合值")], ["Final Value", getText("最终值")]],
				forceSelection:true,
				allowBlank:false,
    ...
    var win = new Ext.Window({
        ...
        items: [Ext.create("Ext.tab.Panel", {
			...
			flex: 1,
			items: [
			{title: getText("一般"), layout: {type:"vbox", align: "stretch"},items: [generalForm, grid],
			autoScroll: true},
			{title: getText("高级"), items: advancedForm,
			autoScroll: true}
			]})
		],
        buttons: [
...
function finishOptimizer(){
	...
	var text = "<div style='padding: 1em; text-align: center;'><p>"+getText("您的模型已设置为最佳解决方案。")+"</p>" + optString(supremeBestPoint);
	
	text = text+"<br/><p>"+getText("请注意，优化算法无法完全考虑局部最小值或最大值。 如果您的解决方案空间包含此类功能，则算法可能会返回错误的结果。")+"</p>"
	if(optimizerController.randomStarts>0 ){
		text = text+"<hr><p>"+getText("您指定了总共 %s 个随机迭代。 初始迭代和每个随机迭代的最优化如下",optimizerController.randomStarts)+":<br/>"
		for(var i=0; i<optimizerController.bestPoints.length; i++){
			text=text+"\n\n"+optString(optimizerController.bestPoints[i])
		}
	}
	
	text=text+"<hr/><p>"+getText("优化持续时间: %s 秒", (Date.now()-optimizerController.startTime)/1000)+"</p></div>"
	...
	for(var i=0; i<optimizerController.evaluations; i++){
		...
		if(optimizerController.startPoints.indexOf(i) != -1 ){
			data[data.length-1].push(getText("迭代开始"));
		}else if(optimizerController.bestPoints.indexOf(i) != -1 ){
			data[data.length-1].push(getText("迭代最优"));
		}else{
